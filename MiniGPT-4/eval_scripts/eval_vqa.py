...
import sys
sys.path.append('/cmlscratch/xic/Poisoning-Vision-Language-Models/MiniGPT-4')
...
parser.add_argument("--configs", type=str, default='', help="config of the evaluated ckpt")
...
if 'vizwiz' in args.dataset:
    ...
    file_save_path = os.path.join(save_path, f"vizwiz-{args.configs}.json")
    with open(file_save_path,'w') as f:
        json.dump(minigpt4_predict, f)
...
if 'gqa' in args.dataset:
    ...
    file_save_path = os.path.join(save_path, f"gqa-{args.configs}.json")
    with open(file_save_path,'w') as f:
        json.dump(minigpt4_predict, f)
